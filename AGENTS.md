...
- Phase: controlled dogfood / beta-hardening on Mac Mini. Core filing is built, but wider invite-only paid beta remains gated by the launch checklist, legal sign-off, and synthetic live proof up to protected attachment boundaries. Deploy: GitHub Actions self-hosted runner, push to `main`, **gated on CI tests passing** with post-deploy smoke + auto-rollback (`deploy_mac.sh`).
...
- Release closure: `scripts/release_loop.sh` is the sole deterministic closure entrypoint, and it runs on **one card, one approval**. Full standard: `docs/release-standard.md`.
  - `--mode prepare --risk internal|telegram|broad --effect "<one line>" [--live-target <bot_username>]` makes no remote, runtime or user-facing change; it refreshes local refs and writes a local card. It runs preflight + offline Telegram QA (with fixed throwaway credentials scoped to those children only), fetches current `origin/main`, verifies the live runtime equals it, then writes one immutable local card under the gitignored `.release/` keyed by the full HEAD SHA: schema version, SHA, surface, risk, effect, proof mode, exact live target, already-verified rollback SHA, `rollback_mode = operator-triggered`, fixed exclusions, timestamp. It prints the card, one exact ship command and one exact rollback command. `--effect` is required, and telegram risk requires an exact `--live-target`. If the tree or live baseline is not release-ready, no card is written and the state is `blocked`; only a verified prepare reports `release-ready`. Prepare is immutable for a SHA: an identical re-prepare reuses the existing card byte for byte and leaves any approval of it standing, while changed effect, target, proof mode, known-good SHA, exclusions or rollback mode — or a hand-edited card — is refused with exit 2 and never rewritten.
  - **Approval binds one SHA and the complete canonical card digest.** `--approved <40hex>:<64hex>` (or the equivalent `RELEASE_APPROVED`) must match both. All card values, including recipient, allowlist, rollback and absolute tool paths, are covered. SHA-only, dated and bare approvals fail closed. Use the exact Git-object-pinned bootstrap command printed by prepare for ship/attest/rollback; an unchanged resume reuses that command and approval without asking again.
  ...
  - Proof mode is frozen on the card. `internal` is automated (CI + deploy + runtime identity, no journey). `telegram` is automated only when Telethon session/API id/hash, `TELEGRAM_BOT_USERNAME` and the live allowlist all name the exact card target; otherwise the card is manual. A manual card never sends automatically, even if credentials appear later. An automated card whose readiness disappears reports pending and refuses the live child rather than downgrading itself. The live guard `TELEGRAM_LIVE_APPROVED` is set per-command on the live child only, never exported; `telegram_bot_qa.sh`'s own direct-call guard is unchanged. The approved target is also passed explicitly as `RELEASE_LIVE_TARGET`; the child captures it before loading `backend/.env` and re-checks the effective username and allowlist afterwards, so a dotenv value cannot redirect an approved live proof. That refusal exits 21 without sending anything and the loop reports `proof-pending`, not a failed journey. `broad` still needs all 15 interactive dogfood checks with no skips.
  - `--mode attest --approved <sha>:<card-digest> --result pass|fail --note "<one line>"` closes only a card already frozen as manual. It re-verifies card, SHA, surface, risk and `HEAD == origin/main == SHA`, then proves the automated half itself — the exact-SHA `push` Tests run, then the `workflow_run` deploy that started after it, then runtime identity — before writing a local attestation, and mutates nothing external. A failed Tests/deploy run is `blocked` and a missing or running one is `proof-pending`; no attestation is written in either case. It says `manual proof attested by operator` and never claims automated proof. It refuses internal risk and every automated card, even if automated readiness later disappears.
  - Workflow proof polls GitHub every `RELEASE_LOOP_PROOF_INTERVAL` seconds (default 60) for up to `RELEASE_LOOP_PROOF_TIMEOUT` seconds per workflow (default 1800). Without a `gh` login it falls back to the public API (60 requests/hour per IP), so it spreads the remaining quota until GitHub's reset time, waits out a rate limit that resets inside the window, and otherwise stops at once with `tests=rate-limited` / `deploy=rate-limited` and `RATE_LIMIT_RETRY_NOT_BEFORE=<UTC>`. A rate limit, an HTTP error or an unreadable payload is never reported as a missing run.
  ...
  - The printed rollback command reuses the same approved card. It is operator-triggered, not silent; deploy-time health rollback is separate and unchanged. It creates one deterministic forward commit with exactly the released parent and frozen known-good tree using `commit-tree`, without changing checkout/index/local branch. Git state is reconciled before retry; `committed`, `pushed`, `proved` journal states are not evidence by themselves. Code comes from approved Git objects through a private temporary bootstrap, never a mutable runner cache. Only exact-SHA CI/deploy/runtime proof permits `rolled-back`; rollback sends no live journey. See `docs/release-standard.md` for trust assumptions and real-Git regression coverage.
...
- **Whole-bot proof**: `bash scripts/telegram_bot_qa.sh --whole-bot` runs the full offline gate, then the reviewed safe live graph and `/cancel` cleanup. Synthetic clinical/input live journeys remain disabled until their isolation is proven, so that layer stays pending rather than being silently skipped or run. One `whole-bot-aggregate.json` gives `passed|pending|failed`; only passed succeeds. Routine testing inside an approved bot/synthetic-account/effect envelope is autonomous, without a second Founder checklist. Missing credentials, skips, unknown controls, uncovered units or absent artifacts never succeed. Live proof remains explicit/on-demand; ordinary CI is offline. The catalogue audit and fake-client/aggregate tests run in `verify_changed.sh` and therefore in release verification. The registration inventory is drift evidence, not behavioural proof. Current registered interfaces/state kinds are exhaustive; language, dynamic values and provider permutations are sampled. Protected effects are exercised only against offline stubs/synthetic storage and observed, never invoked, live. See `docs/telegram-bot-autonomous-testing.md`.
...
- Live Telegram smoke: `cd backend && venv/bin/python3 -m pytest tests/ -v -m live`, or the focused release proof `bash scripts/telegram_bot_qa.sh --focused-release` (`tests/test_e2e.py::test_e2e_cbd_ready_draft_to_cancel_journey`), requires `TELETHON_SESSION`/API credentials and the live allowlist naming the exact bot. This is task-bound autonomy, not a separate manual gate: once Moeed has approved a user-facing Telegram change (or an exact release card) for that bot/recipient/effect, running its safe live verification — clean-state synthetic case through the changed controls to the ready draft, then Cancel — is part of the same unchanged outcome and needs no further approval prompt. A new target, recipient, credential/access change, spend, real patient data, Kaizen save, or broader effect still requires exact foreground approval. This is never continuous monitoring: never run live Telegram tests as routine CI or an unattended/scheduled loop, and always retain a redacted transcript.
...
- **One writer per file set, one directory per writer.** Never work in
  `~/projects/portfolio-guru` itself and never in the live checkout. Claude
  Code sessions use Claude Code's own worktrees (`.claude/worktrees/`, created
  and cleaned up by Claude Code); the gate scripts find the shared dev venv
  (`~/.local/share/portfolio-guru/venv`) there automatically. Other agents
  (Codex, Hermes) use `scripts/new_worktree.sh <short-name>`, which lands in
  `~/projects/.worktrees/` with the same shared venv and `.env`. Either way the
  release loop pushes the exact approved commit to `main`; the branch name is
  only the workspace.
