...
- **Test environment**: the offline gates need `FERNET_SECRET_KEY` exported, because `backend/credentials.py` reads it at import time. Running the gate scripts (`verify_changed.sh`, `preflight.sh`, `verify_release.sh`) handles this for you — `scripts/lib/test_env.sh` uses your real key if set and otherwise generates an ephemeral one for the run, and it fails fast when the virtualenv is missing. Invoking `pytest` directly does neither: with no key you get 14 failures across `test_health_bot.py`, `test_kaizen_login_reliability.py` and `test_flow_walker.py` that look like broken code but are not. Prefer the gate scripts. Git worktrees under `.claude/worktrees/` have no `backend/venv` of their own — symlink the main checkout's.
...
- Release closure: `scripts/release_loop.sh --surface telegram --mode prepare|ship --risk internal|telegram|broad` is the sole deterministic closure entrypoint. `prepare` is side-effect free. `ship` requires an explicit risk class, one semantic approval (`RELEASE_APPROVED=telegram-YYYYMMDD` or `--approved`), and a clean fast-forwardable feature branch. It runs preflight + offline Telegram QA, pushes one exact full SHA, requires a successful `push` Tests run for that SHA, then a later successful `workflow_run` Mac Mini deploy bound to the same SHA, exact checked-out/runtime identity, and risk-scaled proof: `internal` uses automated proof only, `telegram` adds one guarded focused live case journey, and `broad` requires all 15 interactive dogfood checks to pass with no skips. Missing/running/timeout proof exits 4 as `proof-pending`; completed CI/deploy or journey failure is `blocked`. Resume only with the printed `--release-sha <40hex>` command, which requires the same approval and exact local/remote SHA and never pushes twice. Printed proof commands are next gates, not live proof. Do not run approved `ship` unless Moeed has authorised that exact release envelope; once authorised, do not invent another approval inside the unchanged envelope.
