...
- Exercise the affected real product path:
  ...
  - Once Moeed has approved a user-facing Telegram feature/fix (or an exact release card for the same bot/recipient/effect), live verification of that changed journey is task-bound autonomy, not a fresh manual gate: run the focused live proof (`bash scripts/telegram_bot_qa.sh --focused-release`, or the equivalent `-m e2e` node for the changed journey) when Telethon credentials for the approved bot are configured, without asking again. This is never continuous monitoring or routine CI — one run, proving the one approved change. Completion proof for a changed journey means: start from a clean/reset state; send a distinctive synthetic case; read both the sent and received messages; traverse every changed button/state using message-fingerprint (id + text + buttons) change detection, not id ordering alone; assert the final changed message/buttons semantically; exit via Cancel; observe clean cancellation; retain a redacted/local transcript. Fail — do not silently pass or downgrade to offline-only — on a skipped step, a stale/fingerprint-unchanged message, uncertain state, or forbidden/error content. The journey must never save to Kaizen; if a feature can only be proved by saving, stop at that boundary and report proof as pending rather than crossing it. A new target, recipient, credential/access change, spend, real patient data, or broader effect than the approved change still requires exact foreground approval, and missing/incomplete live credentials means reporting proof as pending, not skipping it silently.
...
1. `--mode prepare` passes offline verification, fetches current `origin/main`, proves the live runtime equals it, and writes one local card under the gitignored `.release/`, keyed by the full HEAD SHA and naming surface, risk, the one-line effect, the frozen proof mode, the exact live target, the fixed exclusions, the already-verified rollback target and `rollback_mode = operator-triggered`. It prints the card, one exact ship command and one exact rollback command. It refreshes local refs and writes that local card, but mutates no remote, runtime or user-facing state;
...
3. before main moves, re-fetch current `origin/main`, require it still equals the rollback SHA frozen on the approved card, and re-verify the live runtime against it. Never amend the card after approval; if any value moved, block before mutation and require a new card;
...
Telegram proof is `automated` only when the Telethon session, API id/hash, `TELEGRAM_BOT_USERNAME` and the live allowlist all name the exact card target; otherwise the card is `manual`. A manual card never sends automatically, even if credentials appear afterwards. An automated card whose readiness disappears reports pending rather than running the live child and cannot downgrade to manual attestation; restoring readiness or a new manual card and approval is required. The live guard is supplied per-command to the live child only and is never exported, and the offline gates run with fixed throwaway credentials that the deploy, runtime and live children never inherit. Manual proof closes with `--mode attest --approved <sha> --result pass|fail --note "<one line>"`, which accepts only a card already frozen as manual, re-verifies card, SHA, refs and runtime, records a local attestation, changes nothing external, and reports `manual proof attested by operator` — never automated proof.
