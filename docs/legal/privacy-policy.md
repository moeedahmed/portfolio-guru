...
| **Kaizen signed-in session (passwordless option)**          | If you choose to connect without sharing your password, you sign in to Kaizen yourself on a Portfolio Guru sign-in page and we keep only the resulting signed-in session (browser cookies), not your password. Your typing passes through that page's browser to reach Kaizen but is never stored or logged. | Created when you sign in; stored encrypted (see §5); Kaizen ends it after about a day. «REVIEW: solicitor to confirm wording.» |
...
- **Passwordless option:** no password is stored. The signed-in Kaizen session is encrypted with the same Fernet key, used only to save drafts you approve, replaced each time you sign in again, and deleted by /reset. Choosing this option deletes any previously stored password, including the Supabase mirror copy. «REVIEW: solicitor to confirm.»
...
| Case in progress (not yet saved)          | Kept encrypted (Fernet) on the service machine only so a restart does not lose it. Deleted when you save or cancel the case, after **24 hours** at most, or when you use /reset. «REVIEW: confirm this wording with the solicitor/DPO.» |
