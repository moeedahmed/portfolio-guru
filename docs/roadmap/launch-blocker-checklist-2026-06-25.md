...
> Created 2026-06-25 from the end-to-end production-readiness audit.
...
- [ ] **0.1 (S) Gate deploy on tests.** `deploy-mac.yml` currently deploys on push to `main` with no dependency on `test.yml`. Add `needs:`/`workflow_run` so a red suite blocks deploy.
...
- [ ] **0.2 (S) Fix the plaintext PII leak to cloud.** `extracted_fields` (structured patient detail) is written unencrypted to Supabase, contradicting the in-code "never leaves the bot" claim.
...
- [ ] **0.3 (M) Build a real erasure path.** `/reset` clears local state but `supabase_sync.py` is write-only — cloud copies of credentials, case text, and fields persist. Add `delete_user_data()` and call it from `/reset`; also sweep the document cache + persistence pickle.
...
- [ ] **0.4 (S) Add a daily off-device DB backup.** No backup today = disk failure is total user-data loss.
...
- [ ] **0.5 (S) Operator alerting on the two silent-failure paths.** Today nobody is paged when the bot dies/hangs or a paid webhook is dropped.
...
- [ ] **1.1 (M) Decide and enforce the cross-border LLM posture.** Clinical text defaults to **DeepSeek (China-hosted)**; voice/photo/PDF go to US clouds; there is no de-identification step.
...
- [ ] **2.1 (M) Add subscription reconciliation (no more stranded payers).** Don't rely on the single webhook.
...
- [ ] **2.2 (M) Handle the missing recurring-billing events.** No `invoice.paid`/`payment_succeeded` handler → recovered payments never re-upgrade; failed-payment revokes instantly with no grace.
...
- [ ] **2.3 (M) Prove live Stripe end-to-end + env-consistency guard.** Live keys/price-IDs/webhook-secret must all be the same Stripe environment; a mismatch silently charges-but-doesn't-upgrade today.
...
- [ ] **3.1 (M) Safe deploy: post-deploy smoke + one-command rollback.** `deploy_mac.sh` does `py_compile` only (a syntax check), no rollback path.
...
- [ ] **3.3 (M) Bound concurrent filings.** One shared Chrome, no admission control — N simultaneous approvals spawn N logins. Add an `asyncio.Semaphore`/queue around `route_filing`.
...
- Pin dependencies + add a lockfile (`backend/requirements.txt` is all `>=`, no lock) — unpinned `browser-use`/`playwright` can silently break the auto-deploy.
