...
Portfolio Guru is a 1:1 portfolio/ARCP filing service that sits *behind* the
single EMGurus WhatsApp Gateway — it is not a direct WhatsApp bot. The product
architecture is one WhatsApp business number and one external EMGurus
gateway/router that fans out to the right Guru (Career / Exam / Portfolio).
Group, community, and exam behaviour belong to those other agents behind the
same gateway, never to this service.
...
* **Gateway owns** the WhatsApp business number, Meta/WhatsApp API plumbing,
  DM-vs-group routing, identity resolution, and fan-out to the correct Guru.
...
class SessionRef:
    ...
    ``gateway_user_id`` is the EMGurus identity the gateway resolved, when
